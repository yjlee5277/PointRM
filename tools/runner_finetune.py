...
train_transforms = transforms.Compose(  # 'scan'
    [
         data_transforms.PointcloudRotate(),
    ]
)

train_transforms_raw = transforms.Compose(  # 'm40'
    [
         data_transforms.PointcloudScaleAndTranslate(),
         data_transforms.PointcloudRandomInputDropout(),  ###  TW
         data_transforms.RandomHorizontalFlip(),  ###  TW
         data_transforms.PointcloudScale(),  ###  TW
         data_transforms.PointcloudRotate(),  ###  TW
         data_transforms.PointcloudTranslate(),  ###v  TW
         data_transforms.PointcloudJitter(),  ###  TWs
    ]
)

test_transforms = transforms.Compose(  # 'm40'
    [
        data_transforms.PointcloudScaleAndTranslate(),
    ]
)
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            cos_sim = F.cosine_similarity(map_global_feature.unsqueeze(1).repeat(1, map_patch_feature.shape[1], 1),
                                          map_patch_feature, dim=2)
            loss_CE, loss_GLR, loss_importance, acc = base_model.module.get_loss_acc(ret, label, map_patch_feature,
                                                                                     map_global_feature, cal_importance,
                                                                                     cos_sim)
            ...
            if train_writer is not None:
                train_writer.add_scalar('Loss/Batch/Loss_CE', loss_CE.item(), n_itr)  # lee
                train_writer.add_scalar('Loss/Batch/loss_GLR', loss_GLR.item(), n_itr)  # lee
                train_writer.add_scalar('Loss/Batch/loss_importance', loss_importance.item(), n_itr)  # lee
...
def validate(base_model, test_dataloader, epoch, val_writer, args, config, logger=None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            logits, map_patch_feature_val, map_global_feature_val, cal_importance_val = base_model(points)  # lee
            target = label.view(-1)
